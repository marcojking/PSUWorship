...
const EXPORT_OPTIONS: ExportOption[] = [
  ...
  { id: 'infographic', label: 'Worship Night Flyer', desc: 'WM&A branded' },
];
...
export default function ExportModal({ setlist, songs, onClose }: ExportModalProps) {
  ...
  const exportInfographic = async () => {
    ...
    // Render WM&A title using canvas for proper thin/bold fonts
  ...
  const exportPamphlet = async (flipAlternatePages: boolean = false) => {
    ...
    async function renderPanel(
      ...
      if (panel.type === 'cover') {
        ...
        // Render WM&A title using canvas (scaled by width)
      ...
      if (panel.type === 'back') {
        ...
        try {
          ...
          doc.setFontSize(8);
          doc.setTextColor(0, 48, 73);
          doc.text('WM&A', centerX, height - 30, { align: 'center' });
        }
...
// Render WM&A title using canvas for custom font support
...
// Render WM&A title for pamphlet (scaled version)
